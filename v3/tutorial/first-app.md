...
Выражение `use` в верхней части скрипта добавляет подключения классов `Request` и `Response` в наш скрипт, 
...
файлов, что и я, то `vendor/` каталог находится на один уровень выше вашего `index.php` и вам, потребуется 
настроить путь, как я сделал выше.

Наконец, мы создаем `$app` объект, который является началом Slim goodness. 
Вызов `$app->get()` это наш первый "route" - когда мы делаем GET запрос на `/hello/someone` 
то это код , который ответит на него. **Не забывайте** вам нужна эта заключительная `$app->run()` 
строка, чтобы показать Slim, что мы закончили настройку, и пришло время заняться основным событием.
...
Это наиболее предпочтительный вариант «быстрого старта», потому что он ни от чего не зависит! Из `src/public` 
...
но это сообщение об ошибке **от Slim**, так что это ожидаемо. Попробуйте 
...
Чтобы запустить приложение в стандартном стеке LAMP, нам понадобится пара дополнительных компонентов: 
особая конфигурация виртуального хоста и одно правило перезаписи.

Настройка vhost должна быть довольно простой; нам здесь не нужно ничего особенного. Скопируйте существующую конфигурацию 
vhost по умолчанию и укажите `ServerName` - для ссылки на свой проект. Например, вы можете установить:
...
Затем вы также можете установить `DocumentRoot` чтобы указать `public/` каталог вашего проекта, что-то 
...
**Не забудьте** перезапустить сервер, ведь вы изменили конфигурацию! ( reload будет достаточно )

У меня также есть `.htaccess` файл в моем `src/public` каталоге. Он завязан на том, включен ли модуль 
перезаписи Apache, и просто пересылает все веб-запросы к `index.php`, чтобы Slim мог затем обрабатывать всю 
...
**ПРИМЕЧАНИЕ:** Если вы хотите, чтобы ваша точка входа была чем-то иным, чем index.php, вы можете 
изменить вашу конфигурацию.
`api.php` обычно также может использоваться в качестве точки входа, в таком случае ваши настройки должны быть изменены на необходимые. В нашем примере предполагается, что вы используете `index.php.`

При такой настройке просто не забудьте использовать http://slimproject.dev вместо http://localhost:8080 в 
других примерах этого урока. И ещё, такое же предупреждение об ошибке, как я указывал выше: вы увидите страницу с 
ошибкой на странице http://slimproject.dev, но на самом деле это страница с ошибкой от *Slim*. Если вы перейдете к 
http://slimproject.dev/hello/joebloggs, тогда должно произойти что-то получше.

## Конфигурация и автозагрузчики

Теперь, когда мы настроили платформу, мы можем начать устанавливать все необходимое в самом приложении.

### Добавление параметров конфигурации в приложение

В первом примере используются все значения по умолчанию от Slim, но мы можем легко добавить свою конфигурацию в наше 
приложение, когда мы его создадим. Есть несколько вариантов, но здесь я только что создал массив опций конфигурации, 
а затем сообщил Slim, чтобы он использовал настройки отсюда, когда я создаю его.
...
``` 
Эти строки должны быть добавлены в `src/public/index.php` перед строкой `$app = new \Slim\App`.

Первая строка - самая важная! Включаем режим разработки, чтобы получать информацию об ошибках 
(без этого, Slim будет всего лишь регистрировать ошибки, поэтому, если вы используете встроенный веб-сервер 
PHP, то вы увидите их в консоли, что полезно). Вторая строка позволяет веб-серверу 
устанавливать заголовок Content-Length, который делает Slim более предсказуемым.

Другие настройки здесь не являются конкретными ключами/значениями, это всего лишь мои данные, 
которые я хочу использовать позже.

Теперь, чтобы передать это в Slim, нам нужно *изменить* место где мы создаем`Slim/App` 
объект, чтобы он теперь выглядел следующим образом:

``` php
$app = new \Slim\App(["settings" => $config]);
``` 
Теперь мы сможем получить доступ к любым настройкам, которые мы поместили в массив `$config` внутри нашего приложения.
